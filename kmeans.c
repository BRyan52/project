...
void kmeans(struct my_data *data[DATA_LENGTH], int k, int n, int *output_cluster, double *centroid){
  double prev_total = BIG;
  int iter = 0;

  //dynamically allocate all arrays needed
  double *dist = (double *)malloc(sizeof(double) * (n*k));
  int *curr_cluster = (int *)malloc(sizeof(int) *n);
  int *prev_cluster = (int *)malloc(sizeof(int)*n);
